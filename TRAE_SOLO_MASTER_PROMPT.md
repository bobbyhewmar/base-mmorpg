...
- mini menu fixo no canto inferior direito, identico ao estilo classico, com uma barra azul vertical de drag na esquerda, quatro botoes `32x32px` para Status, Inventory, Map e System, tooltips `Character Status (Alt+T)`, `Inventory (Alt+V)`, `Map (Alt+M)` e `System (Alt+X)`, mapa `ALT+M` sincronizado com o minimapa/top-right por posicao do player e yaw da camera, janela System classica e confirmacao `Do you wish to exit the game?` com `OK` e `Cancel`
- minimapa no canto superior direito, nao-draggable, com projecao ilustrativa da regiao, player centralizado e meia-lua/arco indicando a direcao atual da camera; nao usar minimapa estatico nem inventar autoridade local de mapa
...
- a elegibilidade remota de projection ja nao usa apenas ponto + raio simples: o producer filtra recipients pelo corredor autoritativo de movimento do source, aplica histerese de entrada/retencao sobre a ownership anchor do recipient e emite `despawn` exato quando uma rota antes relevante perde relevancia; tudo continua backend-only, sem known-set distribuido, sem reroute e sem fallback local
...
- `local` nao tem semantica distinta nesta fase e fica explicitamente reservado para depois; a superficie funcional atual exposta e testada e `region`, `party`, `alliance` e `whisper`
...
- a foundation minima de `alliance` agora existe em modo autoritativo: `create_alliance`, `invite_alliance_clan`, `accept_alliance_invite`, `decline_alliance_invite`, `leave_alliance`, `expel_alliance_clan` e `dissolve_alliance` seguem o mesmo command lifecycle replay-safe sem fallback local
- `alliances`, `alliance_members` e `alliance_invites` persistem nome unico, leader clan, membership por clan e convite efemero com TTL de 10s; disconnect de inviter ou invitee cancela o convite e `world/enter.self_state.alliance` mais `self_state.alliance_invites` reidratam a verdade compacta do slice
- alliance e clan-of-clans: o clan fundador entra imediatamente como primeiro member e leader clan, o target atual resolve o leader do clan alvo, o leader clan nao pode usar `leave_alliance`, e `dissolve_alliance` so e permitido quando apenas o leader clan permanece
- `ALT+N` agora tambem projeta o estado minimo de alliance sobre a superficie de clan: `Create Alliance` quando elegivel, nome da alliance mais leader clan e lista compacta de clans membros quando joined, acoes authority-only de `Invite Clan`/`Expel`/`Dissolve Alliance`, e modal dedicado de alliance invite nao-dragavel acima da hotbar com countdown por `expires_at_ms`
- round-robin, master loot, dice UI, redistribution, penalty sofisticada por range/level, clan/alliance reward sharing, siege, clan war amplo, party finder, matchmaking, offline mail, moderacao avancada, clan chat real, command channel, alliance warehouse, clan warehouse, clan skills, academy, subunits, crest rico, privilegios complexos, cooldowns classicos de 24h e transfer manual de leader seguem fora de escopo
...
Prioridade 1: medir e endurecer o efeito do filtro de interest management cross-instance ja refinado por corredor autoritativo + histerese + relevance-loss despawn, alem de ampliar social cross-instance somente com contrato explicito, preservando `presence.target_remote` para combate e sem introduzir Redis/fila externa antes de medir necessidade.
