...
- [x] Remote region-player projection eligibility is narrower than bare same-region ownership: upserts fan out only to exact recipients whose authoritative ownership anchor remains relevant to the source's current point-or-movement corridor under the refined enter/retain hysteresis rule.
- [x] A recipient that was previously relevant but leaves the refined projection interest rule receives an exact-recipient despawn ordered in the same source fence/version stream, so interest loss does not wait only for TTL and stale upserts cannot resurrect visibility afterward.
