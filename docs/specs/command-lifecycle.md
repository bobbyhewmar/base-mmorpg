...
- before applying a new PvP mutation, the backend may first apply any due deterministic karma-recovery step for attacker or victim from durable schedule state; this is still backend-owned state mutation, not a client timer
- a successful player hit locks both durable character rows in deterministic order and atomically checkpoints combat resources, exposure deadlines, attacker cooldown, lethal victim cleanup, PvP/PK consequences, attribution/anti-feed fields, due karma-recovery state, one combat audit event, and any resulting recovery-audit rows before emitting the correlated actor delta
- target is in range
...
Remote social delivery uses the same command collector and may finalize more than one recipient event. Each key derives from `session_id + command_seq + purpose + recipient_character_id`. The command outcome, sanitized chat history, and complete remote event set commit together for whisper, region, and party chat; failure replaces sender success with `system.persistence_failed`. Region-chat local fanout also waits for commit and revalidates the recipient's local region before socket delivery. Party-chat remote delivery additionally captures the authoritative `party_id` and revalidates it on the destination before socket delivery. Command-driven party/clan mutation, final command outcome, and all resulting remote notices share one transaction. The earlier pending dedup reservation may survive a pre-mutation crash, but an applied mutation cannot commit without its outcome/outbox.
...
For `send_chat_message`, the same authoritative commit boundary is responsible for persisting minimum chat history in `chat_messages`. Delivery scope remains server truth derived from current region, party membership, current alliance membership derived from clan truth, canonical character identity, and durable ownership. A remote whisper stores chat history, command outcome, and one `social.chat_message.v1` delivery intent atomically. Region chat stores the same history/outcome plus one exact-owner event per remote recipient and delivers to still-eligible local recipients only after commit. Party chat stores the same history/outcome plus one exact-owner event per remote party member that is currently online/attached on another instance and delivers to still-eligible local party members only after commit. Alliance chat stores the same history/outcome plus one exact-owner event per remote allied recipient and delivers to still-eligible local allied recipients only after commit. The current slice exposes `region`, `party`, `alliance`, and `whisper`, and every scope remains server-resolved. `local` remains reserved for a later distinct scope. The client never authors the final recipient set.

For player combat, a process-local mutex may coordinate runtime projection but cannot be the correctness boundary. PostgreSQL-backed mode serializes attacker/victim mutations through deterministic row locks and computes damage, death classification, counters, deadlines, cooldown mutation, attribution, repeated-pair signal, account-first correlation inputs, and audit from the locked durable state. The same durability boundary may also apply due karma recovery and persist `pvp_karma_events` when a due step exists. The memory adapter mirrors this in one critical section. The generic post-command progression/cooldown flush must not run after this transaction, because it could overwrite a newer multi-instance combat state.
